  ...
  <body class="p-3 m-0 border-0 bd-example">
    ...
    <center>
      <div class="card mb-3" style="max-width: 58rem; background-color:rgba(0,0,0,0.1);top: 50%; left: 50%; transform: translateY(-50%) translateX(-50%); position: absolute">
        ...
        <p></p>
        <form method="post" class="form-signin" action="<%=request.getContextPath()%>/ServletInicio?action=establecernuevacontrausuario">
          ...
          <br>
          <p></p>
          <% if (session.getAttribute("errorUsuar") != null) {%>
          <div class="text-danger mb-2"><%=session.getAttribute("errorUsuar")%></div>
          <% session.removeAttribute("errorUsuar"); %>
          <% } %>
          <br>
